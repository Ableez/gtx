...
import { CheckBadgeIcon, PaperAirplaneIcon } from "@heroicons/react/20/solid";
...
const UserChatScreen = ({ params }: Props) => {
        ...
        fetchChatData().then(() => {
          if (chatsAutoScroll.current) {
            chatsAutoScroll.current.scrollTo({
              top: chatsAutoScroll.current.scrollHeight + 1000,
              behavior: "smooth",
            });
          }
        });
  ...
  const messagesBubble = document.querySelectorAll("#message-bubble");
    ...
    return messages?.messages.map((message, idx) => {
      return (
        <li
          id="message-bubble"
          key={idx}
          className={`flex leading-5 last-of-type:mb-4 ${
        ...
        >
          <div
            ...
            } relative max-w-[320px] md:max-w-[560px] ${
              message.media
            ...
            {message.media !== "" ? (
              <>
                <img
                  ...
                  className="rounded-md w-auto h-auto min-h-max hover:scale-[1.024] duration-200 cursor-pointer aspect-square object-cover"
                />
              </>
            ) : (
    ...
    >
      ...
      <AlertDialog open={view.open}>
        <AlertDialogContent className="pt-16">
          <AlertDialogCancel
            className="w-fit absolute top-10 p-5 right-3 dark:bg-neutral-800"
            onClick={() => {
              setView((prev) => {
                return {
                  ...prev,
